<template>
    ...
    <div class="d-flex bannerAdmin">
      <div :class="{ active: index === 0 }" v-for="(banner, index) in banners" :key="index">
        ...
        <img :src="banner.url" :alt="banner.provider" />
      </div>
    ...
    methods: {
      async getBannerData() {
        try {
          ...
          const response = await this.$axios.get('getAllGallery');
      ...
      async removeBanner(bannerId) {
        ...
        if (this.banners) {
          ...
          if (index !== -1) {
            ...
            await this.$axios.delete(`deleteGallery/${bannerId}`)
              .then((response) => {
      ...
      async submitForm() {
        try {
          if (this.filesArr.length === 0) {
            ...
            const response = await this.$axios.post('addGallery', formData, {
              headers: {
