<template>
    <div class="container-xxl bg-white p-0">
        ...
        <div class="breadcrumb">
            ...
            <span class="breadcrumb-part">Chi tiết Khách sạn đã xem</span>
...
export default {
    ...
    methods: {
        ...
        async getDetailUserLike() {
            try {
                const response = await this.$axios.get(`getseen/${this.userInfo.id}`);
        ...
        DetailHotel(hotel) {
            console.log(hotel.maKhachSan);
            const SeenHotels = JSON.parse(localStorage.getItem('SeenHotels') || '[]');
            const hotelData = SeenHotels.find(item => item.id === hotel.maKhachSan);
