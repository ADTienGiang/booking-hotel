  ...
  //url trang chủ `${process.env.VITE_API_BASE_URL_API_FRONTEND}` = 5173
...
routerAPI.get('/auth/facebook/callback',
  ...
  function(req, res) {
    ...
    const redirectUrl = `${process.env.VITE_API_BASE_URL_API_FRONTEND}?token_fb=${token}&user_inf_fb=${JSON.stringify(user)}`;
...
routerAPI.get('/auth/google/callback',
  ...
  function(req, res) {
    ...
    const redirectUrl = `${process.env.VITE_API_BASE_URL_API_FRONTEND}?token_gg=${token}&user_inf_gg=${JSON.stringify(user)}`;
    res.redirect(redirectUrl);
  }
);
